...
- CVE-2026-53362 - ip6frag_escape unprivileged LPE and container breakout. The IPv6 `__ip6_append_data()` fraggap mis-accounting on the MSG_SPLICE_PAGES paged path overflows in-slab into skb_shared_info, giving nr_frags control → pipe page UAF → Dirty-Pagetable → core_pattern root shell in the init namespaces. Ubuntu 24.04 vmm/Firecracker PoC (custom 6.12.87 kernel), 4-level paging port of the original RHEL 10 PoC. Also demonstrated from an unprivileged user inside a network-isolated Docker container.
